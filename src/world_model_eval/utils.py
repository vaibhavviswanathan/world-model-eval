...
""".strip()
    return prompt, has_partial, allowed_pattern


def _tally_votes(texts, allowed_pattern, has_partial, n):
...
def _predict_anthropic(frames, trial, n, model):
    import anthropic
    client = anthropic.Anthropic()
    prompt, has_partial, allowed_pattern = _build_prompt(trial)
    ...
    texts = []
    for _ in range(n):
        res = client.messages.create(model=model, max_tokens=512, messages=messages)
        texts.append(res.content[0].text)

    return _tally_votes(texts, allowed_pattern, has_partial, n)


def _predict_openai(frames, trial, n, model):
    ...
    res = client.chat.completions.create(model=model, messages=messages, n=n)
    texts = [(c.message.content or "") for c in res.choices]

    return _tally_votes(texts, allowed_pattern, has_partial, n)


def predict(video, trial, n=5, judge="anthropic"):
    frames = encode_video(video)
    if judge.startswith("openai"):
        # e.g. "openai" or "openai:gpt-4o"
        model = judge.split(":", 1)[1] if ":" in judge else "gpt-4o"
        return _predict_openai(frames, trial, n, model)
    else:
        # e.g. "anthropic" or "anthropic:claude-sonnet-4-5-20250929"
        model = judge.split(":", 1)[1] if ":" in judge else "claude-sonnet-4-5-20250929"
        return _predict_anthropic(frames, trial, n, model)
